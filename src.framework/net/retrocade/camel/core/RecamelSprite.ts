...
export class RecamelSprite extends PIXI.Sprite {
	...
	public alignCenter(offset: number = 0): void {
		this.x = ((S.SIZE_GAME_WIDTH - this.getLocalBounds().width) / 2 | 0) + offset;
	}
	...
	public alignCenterParent(offset: number = 0, width: number = NaN): void {
		...
		this.x = ((width - this.getLocalBounds().width) / 2 | 0) + offset;
	}
	...
	public alignMiddle(offset: number = 0): void {
		this.y = ((S.SIZE_GAME_HEIGHT - this.getLocalBounds().height) / 2 | 0) + offset;
	}
	...
	public alignMiddleParent(offset: number = 0, height: number = NaN): void {
		...
		this.y = ((height - this.getLocalBounds().height) / 2 | 0) + offset;
	}
	...
	public get bottom(): number {
		return this.y + this.getLocalBounds().height;
	}
	...
	public set bottom(newVal: number) {
		this.y = newVal - this.getLocalBounds().height;
	}
	...
	public get center(): number {
		return this.x + this.getLocalBounds().width / 2;
	}
	...
	public set center(value: number) {
		this.x = value - this.getLocalBounds().width / 2;
	}
	...
	public get middle(): number {
		return this.y + this.getLocalBounds().height / 2;
	}
	...
	public set middle(value: number) {
		this.y = value - this.getLocalBounds().height / 2;
	}
	...
	public get right(): number {
		return this.x + this.getLocalBounds().width;
	}
	...
	public set right(newVal: number) {
		this.x = newVal - this.getLocalBounds().width;
	}
